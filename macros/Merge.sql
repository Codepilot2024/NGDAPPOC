{% macro snowflake__get_merge_sql(target, source_sql, unique_key, dest_columns, incremental_predicates) -%}
    ...
    #}
    {%- set insert_exclude_columns = config.get('insert_exclude_columns') -%}
    {%  set filtered_col= dest_columns| selectattr('name','ne',insert_exclude_columns)  %}
    {%- set dest_cols_csv = get_quoted_csv(filtered_col | map(attribute='name')) -%}
